...
class Publisher_Extend_Featured_Image {
	...
	public function init() {
		...
		if ( ! class_exists( 'Publisher_Screenshot_Manager' ) ) {
			...
			require Publisher_Theme_Core()->get_dir_path( 'featured-image/fetch-screenshot/class-publisher-screenshot-manager.php' );
		}
	...
	public function catch_video_post_thumbnail( $post_id ) {
		...
		if ( ! $video_url = get_post_meta( $post_id, '_featured_embed_code', TRUE ) ) {
		...
		$provider = Publisher_Fetch_Screenshot_util::detect_service_provider( $video_url );
		$video_id = array_shift( $id_list );

		if ( ! $attachment_id = $this->get_attachment_video_screenshot( $video_id, $provider ) ) {

			$attachment_id = $this->catch_screenshot( $video_url, $video_id, $post_id, $provider );
		}
	...
	protected function save_screenshot( $thumbnail_url, $video_url, $video_id, $post_id, $provider ) {
		...
		$attachment_id = media_handle_sideload(
			$file_array,
			$post_id,
			sprintf( "Automatic downloaded screenshot from %s by Publisher", $video_url )
		);
