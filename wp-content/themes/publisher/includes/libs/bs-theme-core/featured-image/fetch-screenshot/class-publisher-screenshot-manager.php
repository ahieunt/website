...
class Publisher_Screenshot_Manager {
	...
	protected $providers_list = array(
		'vimeo'       => 'Publisher_Fetch_Screenshot_Vimeo',
		'youtube'     => 'Publisher_Fetch_Screenshot_Youtube',
		'twitter'     => 'Publisher_Fetch_Screenshot_Twitter',
		'facebook'    => 'Publisher_Fetch_Screenshot_Facebook',
		'dailymotion' => 'Publisher_Fetch_Screenshot_DailyMotion',
	);
	...
	public function __construct( $video_url = array() ) {
		...
		if ( ! class_exists( 'Publisher_Fetch_Screenshot_util' ) ) {
			...
			require dirname( __FILE__ ) . '/class-publisher-fetch-screenshot-util.php';
		}

		if ( ! class_exists( 'Publisher_Fetch_Screenshot_Service_Interface' ) ) {

			require dirname( __FILE__ ) . '/class-publisher-fetch-screenshot-service-service-interface.php';
		}
	}
	...
	public function set_video_url_list( $url_list ) {
		...
		foreach ( (array) $url_list as $url ) {
			...
			$this->push_single_video_url( $url );
		}
	}
